...
2. 薪資統一轉換為「月薪（萬）」填入 s10/s12/s14/s15/s16 欄位：
   ...
   - 如果是班薪制（如每班 2.4~3.2 萬），一律用「**基本班薪** × 班數」填入對應欄位（未指明班數時預設 15 班→s15）。基本班薪＝貼文明寫的每班固定金額；**假日／夜班加給、看診費、收治費、PPF、免稅項目等變動獎勵一律不加進數字**，改寫進 note。若貼文只給班薪區間（如 2.4~3.2 萬），取區間中位數並在 note 註明區間。⚠️ 同一篇貼文在不同日期重抓時**必須算出相同數字**——換算方式不可因次而異，否則下游會誤判成「薪資變動」而重複入庫
...
5. 醫院名稱一律用**正式全名**，**禁止用兩字簡稱**。貼文只寫簡稱時請展開：「部桃」→「部立桃園醫院」、「部苗」→「部立苗栗醫院」、「署新」→「部立新營醫院」、「北榮」→「臺北榮民總醫院」、「員基」→「員林基督教醫院」、「中醫北港」→「中國醫藥大學北港附設醫院」。⚠️ 兩字簡稱會讓下游醫院比對（需 ≥3 字包含）直接失效，導致同一家醫院被重複建立
